...
get git commits
for each commit
get id, check if we have seen it if not, then isnert it into the seen hashmap
build the helix commit from the helix commit
get commit metadata
get parent commits
by parent_id, look up the helix hash in the git_hash_to_helix_hash map using the git hash to get the helix hash
so now we have a vector of parent hashes by their helix_hash
once built, then insert into the git hash to helix hash map
push the helix commit into the helix_commits vector
